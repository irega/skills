...
|-------|-----------|---------|------------------|
| **Skill** | A workflow with steps and exit criteria | `irega-code-review` | The *how* — invoked from inside a persona or command |
| **Persona** | A role with a perspective and an output format | `code-reviewer` | The *who* — adopts a viewpoint, produces a report |
| **Command** | A user-facing entry point | `/irega-code-review`, `/ship` | The *when* — composes personas and skills |
...
> **Status in this repo:** the `code-reviewer` and `security-auditor` personas are wired to load their skills (`irega-code-review`, `security-audit`) via the "Persona loads Method" pattern. The `/ship` fan-out orchestrator described below is **aspirational** — it is documented as the endorsed pattern but is not yet built as a command here. `orchestration-patterns.md` lives in the upstream [agent-skills](https://github.com/addyosmani/agent-skills) repo, not this one.
...
- `/irega-code-review` → the review skill the `code-reviewer` persona loads
- `/security-audit` → the audit skill the `security-auditor` persona loads
- `/tdd` → red-green-refactor workflow the `test-engineer` persona can load
...
This is the only orchestration pattern this repo endorses. See [orchestration-patterns.md (upstream)](https://github.com/addyosmani/agent-skills/blob/main/references/orchestration-patterns.md) for the full pattern catalog and anti-patterns.
...
5. If the persona enables a new orchestration pattern, document it in this README (or a dedicated reference) rather than inventing the pattern in the persona file itself.
